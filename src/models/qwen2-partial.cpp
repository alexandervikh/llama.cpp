...
// Partial layer-range Qwen2 graph builder. Mirrors llm_build_llama_partial but
// adapted for the Qwen2 architecture (no rope_factors, no kq_norm, no bias on W_O).
//
// When il_start > 0 the embedding lookup is bypassed: the caller must populate
// ubatch.embd with the residual-stream values from an earlier partial-score call.
//
// When il_end < n_layer the final output norm and lm_head are skipped; the
// residual stream after layer (il_end-1) is left named "l_out-N" and exposed
// as res->t_embd. Callers extract it via get_gf_res_prev() -> find_named_tensor().

llm_build_qwen2_partial::llm_build_qwen2_partial(const llama_model & model, const llm_graph_params & params)
    ...
    if (last_chunk) {
        ...
        // Intermediate chunk: expose residual stream as t_embd.
        // The tensor already carries the "l_out-N" name — do NOT rename here.
        res->t_embd = cur;
    }
